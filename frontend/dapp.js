...
const contractAddress = "0x2534D2E645a4a172f405c7B1A6CF0ce2e48FCbaF";
...
const dApp = {
  ...
  collectVars: async function () {
    ...
    const fetchMetadata = (reference_uri) =>
      fetch(
        `https://gateway.pinata.cloud/ipfs/${reference_uri.replace(
          "ipfs://",
          ""
        )}`,
        { mode: "cors" }
    ...
    for (let i = 1; i <= this.totalSupply; i++) {
      try {
        ...
        this.tokens.push({
          tokenId: i,
          highestBid: Number(
            await this.marsContract.methods.highestBid(i).call()
          ),
          auctionEnded: Boolean(
            await this.marsContract.methods.auctionEnded(i).call()
          ),
          pendingReturn: Number(
            await this.marsContract.methods
              .pendingReturn(i, this.accounts[0])
              .call()
          ),
          ...
          `<div class="col m6">
              <div class="card">
                <div class="card-image">
                  <img id="dapp-image" src="https://gateway.pinata.cloud/ipfs/${token.image.replace(
                    "ipfs://",
                    ""
                  )}">
                ...
                <div class="card-action">
                  <input type="number" min="${
                    token.highestBid + 1
                  }" name="dapp-wei" value="${token.highestBid + 1}" ${
            token.auctionEnded ? "disabled" : ""
          }>
  ...
  bid: async function (event) {
    ...
    await this.marsContract.methods
      .bid(tokenId)
      .send({ from: this.accounts[0], value: wei })
  ...
  withdraw: async function (event) {
    ...
    await this.tokens[tokenId].auction.methods
      .withdraw()
      .send({ from: this.accounts[0] })
      .on("receipt", async (receipt) => {
        M.toast({ html: "Transaction Mined! Refreshing UI..." });
  ...
  registerLand: async function () {
    ...
    try {
      ...
      const json_upload_response = await fetch(
  ...
  main: async function () {
    ...
    this.isAdmin =
      this.accounts[0] == (await this.marsContract.methods.owner().call());
